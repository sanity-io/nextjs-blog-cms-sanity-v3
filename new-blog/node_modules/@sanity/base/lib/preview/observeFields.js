...
var getGlobalEvents = () => {
  if (!_globalListener) {
    ...
    var welcome$ = allEvents$.pipe((0, _operators.filter)(event => event.type === 'welcome'), (0, _operators.publishReplay)(1), (0, _operators.refCount)());
...
var getBatchFetcherForDataset = (0, _memoize2.default)(function getBatchFetcherForDataset(apiConfig) {
  var client = _versionedClient.versionedClient.withConfig(apiConfig);
  var fetchAll = fetchAllDocumentPathsWith(client);
  return (0, _debounceCollect.debounceCollect)(fetchAll, 10);
}, apiConfig => apiConfig.dataset + apiConfig.projectId);
var CROSS_DATASET_PREVIEW_POLL_INTERVAL = 10000;
...
function crossDatasetListenFields(id, fields, apiConfig) {
  return visiblePoll$.pipe((0, _operators.startWith)(0)).pipe((0, _operators.switchMap)(() => {
    var batchFetcher = getBatchFetcherForDataset(apiConfig);
...
function cachedObserveFields(id, fields, apiConfig) {
  ...
  return (0, _rxjs.combineLatest)(cachedFieldObservers).pipe(
  ...
  (0, _operators.map)(snapshots => snapshots.filter(Boolean)),
  // make sure all snapshots agree on same revision
  (0, _operators.filter)(snapshots => (0, _isUniqueBy.default)(snapshots, snapshot => snapshot._rev)),
  // pass on value with the requested fields (or null if value is deleted)
  (0, _operators.map)(snapshots => snapshots.length === 0 ? null : pickFrom(snapshots, fields)),
