...
function navigate(nextUrl, options) {
  ...
  var debugParams = (currentUrl.hash || '').substring(1).split(';').filter(param => param.startsWith('_debug_'));
  var finalUrl = nextUrl + (debugParams.length > 0 ? "#".concat(debugParams.join(';')) : '');
  if (options.replace) {
