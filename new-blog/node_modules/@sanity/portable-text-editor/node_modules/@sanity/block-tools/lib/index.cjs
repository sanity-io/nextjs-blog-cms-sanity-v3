...
function blockContentFeatures(blockContentType) {
    ...
    var ofType = (_b = (_a = blockType.fields.find(types.isBlockChildrenObjectField)) === null || _a === void 0 ? void 0 : _a.type) === null || _b === void 0 ? void 0 : _b.of;
    if (!ofType) {
    ...
    var inlineObjectTypes = ofType.filter(function (inlineType) {
        return inlineType.name !== 'span' && types.isObjectSchemaType(inlineType);
    });
    var blockObjectTypes = blockContentType.of.filter(function (memberType) {
        return memberType.name !== blockType.name && types.isObjectSchemaType(memberType);
    });
    return {
...
function resolveEnabledStyles(blockType) {
    var styleField = blockType.fields.find(types.isStyleObjectField);
    if (!styleField) {
...
function resolveEnabledListItems(blockType) {
    var listField = blockType.fields.find(types.isListObjectField);
    if (!listField) {
...
function getTitledListValuesFromEnumListOptions(options) {
    ...
    return list.map(function (item) { return (types.isTitledListValue(item) ? item : { title: item, value: item }); });
}
            ...
            if (_this._markDefs.length > 0) {
                blocks
                    ...
                    block.markDefs = block.markDefs.concat(_this._markDefs.filter(function (def) {
                        return lodash.flatten(block.children.map(function (child) { return child.marks || []; })).includes(def._key);
                    }));
                });
            }
