...
function validateInputComponent(typeDef) {
    if ('components' in typeDef && !isInputComponentLike(typeDef.components.input)) {
        return [
            warning("The `components.input` property is set but does not appear to be a valid React component (expected a function, but saw ".concat(inspect(typeDef.components.input), "). If you have imported a custom input component, please verify that you have imported the correct named/default export.")),
        ];
    }
    return [];
}
...
var object = (function (typeDef, visitorContext) {
    ...
    return __assign(__assign({}, typeDef), { preview: preview, fields: (Array.isArray(typeDef.fields) ? typeDef.fields : []).map(function (field, index) {
            ...
            return __assign(__assign({ name: name }, fieldType), { _problems: validateField(field).concat(_problems || []) });
        }), _problems: problems });
});
...
var reference = (function (typeDef, visitorContext) {
    ...
    problems.push.apply(problems, __spreadArray([], __read(getOptionErrors$1(typeDef)), false));
    return __assign(__assign({}, typeDef), { to: (isValidTo ? normalizedTo : []).map(visitorContext.visit), _problems: problems });
});
...
var image = (function (typeDef, visitorContext) {
    ...
    if (typeof metadata !== 'undefined' && !Array.isArray(metadata)) {
        ...
        options = __assign(__assign({}, options), { metadata: metadata.filter(function (meta) { return !autoMeta.includes(meta); }) });
    }
    return __assign(__assign({}, typeDef), { options: options, fields: (Array.isArray(fields) ? fields : []).map(function (field, index) {
            var name = field.name, fieldTypeDef = __rest(field, ["name"]);
            var _a = visitorContext.visit(fieldTypeDef, index), _problems = _a._problems, fieldType = __rest(_a, ["_problems"]);
            return __assign(__assign({ name: name }, fieldType), { _problems: validateField(field).concat(_problems || []) });
        }), _problems: problems });
...
function validateTypeName(typeName, visitorContext) {
    ...
    if (!typeName) {
        return [
            error("Type is missing a type. Valid types are: ".concat(humanizeList(possibleTypeNames)), HELP_IDS.TYPE_MISSING_TYPE),
        ];
    }
    if (typeof typeName !== 'string') {
        return [
            error("Type has an invalid \"type\"-property - should be a string. Valid types are: ".concat(humanizeList(possibleTypeNames)), HELP_IDS.TYPE_MISSING_TYPE),
        ];
    }
    ...
    if (!isValid) {
        ...
        var suggestion = suggestions.length > 0
            ? " Did you mean ".concat(humanizeList(suggestions.map(quote), { conjunction: 'or' }), "?")
            : '';
        return [
...
var common = (function (typeDef, visitorContext) {
    return __assign(__assign({}, typeDef), { _problems: __spreadArray(__spreadArray([], __read(validateTypeName(typeDef.type, visitorContext)), false), __read(validateNonObjectFieldsProp(typeDef, visitorContext)), false).filter(Boolean) });
});
