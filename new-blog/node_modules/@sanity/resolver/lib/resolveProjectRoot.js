...
function resolveProjectRootSync(options) {
  var manifestDir = options.basePath;
  var isProjectRoot = isRoot(manifestDir, options);
  while (!isProjectRoot && _path.default.dirname(manifestDir) !== manifestDir) {
    manifestDir = _path.default.dirname(manifestDir);
...
function isRoot(manifestDir, options) {
  try {
    var manifest = (0, _readManifest.default)(Object.assign({}, options, {
      manifestDir,
      sync: true
    }));
