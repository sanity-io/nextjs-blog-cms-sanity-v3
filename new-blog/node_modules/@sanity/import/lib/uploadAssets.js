...
async function uploadAssets(assets, options) {
  ...
  debug('Uploading assets with a concurrency of %d', concurrency); // Build a Map where the keys are `type#url` and the value is an array of all
  ...
  const ensureMethod = options.allowFailingAssets ? (...args) => ensureAssetExists(...args).catch(err => err) : ensureAssetExists; // Loop over all unique URLs and ensure they exist, and if not, upload them

  const mapOptions = {
    concurrency
  };
  const assetIds = await pMap(assetRefMap.keys(), ensureMethod, mapOptions); // Extract a list of all failures so we may report them and possibly retry them later

  const assetFailures = getUploadFailures(assetRefMap, assetIds); // Loop over all documents that need asset references to be set
...
async function ensureAssetWithRetries(options, progress, assetKey, i) {
  ...
  const {
    buffer,
    sha1hash
  } = await retryOnFailure(() => downloadAsset(url, i)).catch(err => {
    progress();
    err.type = type;
    err.url = url;
    err.message = err.message.includes(url) ? err.message : `Failed to download ${type} @ ${url}:\n${err.message}`;
    throw err;
  });
  const asset = {
    buffer,
  ...
  return retryOnFailure(() => ensureAsset(asset, options, i)).then(progress).catch(err => {
    progress();
    err.type = type;
    err.url = url;
    err.message = err.message.includes(url) ? err.message : `Failed to upload ${type} @ ${url}:\n${err.message}`;
    throw err;
  });
}
...
async function ensureAsset(asset, options, i) {
  ...
  const {
    ...
    replaceAssets,
    tag
  } = options; // See if the item exists on the server
...
function reducePatch(trx, task) {
  return trx.patch(task.documentId, patch => patch.setIfMissing({
    [task.path]: {
