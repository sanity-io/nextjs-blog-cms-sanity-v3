...
module.exports = async function validateAssetDocuments(docs, options) {
  ...
  assetDocs.forEach(doc => validateAssetDocumentProperties(doc)); // Don't allow assets that reference different datasets (unless explicitly allowing it)
...
function validateAssetDocumentProperties(assetDoc) {
  Object.keys(REQUIRED_PROPERTIES).forEach(prop => {
    ...
    if (typeof assetDoc[prop] !== expectedType) {
      const errorType = typeof assetDoc[prop] === 'undefined' ? 'is missing' : 'has invalid type for';
...
function validateImageMetadata(assetDoc) {
  ...
  dimensionProps.forEach(prop => {
    if (typeof assetDoc.metadata.dimensions[prop] !== 'number') {
      throw new Error(`Asset document ${assetDoc._id} is missing required property "metadata.dimensions.${prop}"`);
    }
  });
}