...
var adapter = 'node'; // Reduce a fully fledged node-style response object to
...
module.exports = function (context, cb) {
  ...
  var tunnel = proxy && tunneling.shouldEnable(options); // Allow middleware to inject a response, for instance in the case of caching or mocking
  ...
  if (!tunnel && proxy && proxy.auth && !reqOpts.headers['proxy-authorization']) {
    ...
    var auth = Buffer.from("".concat(username, ":").concat(password), 'utf8');
  ...
  var _getProgressStream = getProgressStream(options),
      ...
      progress = _getProgressStream.progress; // Let middleware know we're about to do a request
...
function getRequestTransport(reqOpts, proxy, tunnel) {
  ...
  var transports = reqOpts.maxRedirects === 0 ? {
    http: http,
    https: https
  } : {
    http: follow.http,
